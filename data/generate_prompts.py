...
def generate_prompts(
    ...
    for product_id, data in metadata_partition.items():
        ...
        if title and categories:
            ...
            assistant_content = f"<explain>This product belongs to {category_text} category/brand. Key features include: {description_text}</explain>"
    ...
    print(f"Generated {len(prompts)} product analysis prompts")
    ...
    for user_id, user_data in review_partition.items():
        ...
        if len(product_ids) >= 2:  # Need at least 2 products for history
            ...
            purchased_products = []
            next_product_id = sorted_product_ids[-1]  # Last product as prediction target
            ...
            if purchased_products and next_product_id in metadata_partition:
                ...
                assistant_content = f"<explain>The customer will purchase {next_title} that belongs to {category_text} category or brand. The product has following features: {description_text}</explain>"
    ...
    print(f"Generated {prediction_prompts} purchase prediction prompts")
    ...
    print(f"Successfully generated {len(prompts)} prompts and saved to {path_to_save_prompt}")
    return prompts
